...
const SUPABASE_URL =
  process.env.SUPABASE_URL || "https://fxdhgcbbdapuzeymoxro.supabase.co";
const SUPABASE_KEY =
  process.env.SUPABASE_KEY ||
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ4ZGhnY2JiZGFwdXpleW1veHJvIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM0MjIwMDAsImV4cCI6MjA4ODk5ODAwMH0.namqHjEMcwlOOsy6U5-t3G9xNxYmPSTgPyOlC5lpfyA";
...
app.post("/track", async (req, res) => {
  ...
  console.log(
    `[TRACK] stage="${stage}"${email ? ` email="${email}"` : ""}${roles ? ` roles=${JSON.stringify(roles)}` : ""}`,
  );
  res.json({ ok: true });
});
...
app.get("/results", async (req, res) => {
  try {
    ...
    // build email list with role + download cross-reference
    const emailList = emailEvents.map((e) => ({
      email: e.email,
      roles: e.roles || {},
      downloadedFile: events.some(
        (ev) => ev.stage === "file_downloaded" && ev.email === e.email,
      ),
